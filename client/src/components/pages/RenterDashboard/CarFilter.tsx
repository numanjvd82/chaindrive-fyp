...
const CarFilter: React.FC<CarFilterProps> = ({
  ...
  console.log("rendering CarFilter with rentals:", availableRentals.length);
  ...
  const yearOptions = [
    ...
    ...Array.from(
      new Set(availableRentals.map((car) => car.year))
    )
      .sort((a, b) => b - a)
      .map((year) => ({
        value: year.toString(),
    ...
    <div className="mb-6 relative">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
        <Button
          ...
          disabled={isLoading}
        >
          <FaFilter className={`transition-transform duration-200 ${isFilterOpen ? 'rotate-180' : ''}`} />
          <span className="hidden sm:inline">{isLoading ? 'Loading...' : 'Filters'}</span>
          <span className="sm:hidden">{isLoading ? 'Loading...' : 'Filter Cars'}</span>
          ...
          <div className="absolute top-full left-0 right-0 z-50 bg-white p-6 rounded-lg shadow-2xl border border-gray-200 mt-2 max-h-96 overflow-y-auto">
            ...
            <div>
              ...
              <Select
                ...
                onChange={(e) => handleFilterChange("numOfSeats", e.target.value)}
